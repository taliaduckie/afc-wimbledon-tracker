"""
Streamlit dashboard for AFC Wimbledon & Restaurant Tracker.
...
# --- Sidebar navigation ---
page = st.sidebar.radio("Navigate", ["AFC Wimbledon", "Restaurants"])

# =============================================================================
# AFC WIMBLEDON
# =============================================================================
if page == "AFC Wimbledon":
    st.title("\u26bd AFC Wimbledon Tracker")
    st.caption("GO WOMBLES")

    if not RESULTS_PATH.exists():
        st.warning("No results found. Run `python -m src afc fetch` first.")
        st.stop()
...
# =============================================================================
    ...
    st.title("\U0001f37d\ufe0f Restaurant Difficulty Index")

    if not RESTAURANTS_PATH.exists():
        st.warning("No restaurants tracked yet.")
        st.stop()

    rdf = pd.read_csv(RESTAURANTS_PATH)

    if rdf.empty:
        st.info("No restaurants tracked yet. Add some with `python -m src restaurant add`.")
        st.stop()

    rdf["difficulty_score"] = rdf["difficulty_score"].astype(float)
    rdf = rdf.sort_values("difficulty_score", ascending=False).reset_index(drop=True)

    # --- Summary ---
    st.header("Overview")
    c1, c2, c3 = st.columns(3)
    c1.metric("Tracked", len(rdf))
    c2.metric("Avg Difficulty", f"{rdf['difficulty_score'].mean():.1f}/10")
    c3.metric("Hardest", rdf.iloc[0]["name"])

    # --- Chart ---
    st.header("Difficulty Scores")
    fig, ax = plt.subplots(figsize=(10, max(3, len(rdf) * 0.5)))
    colors = rdf["difficulty_score"].apply(
        lambda s: "#e74c3c" if s >= 7 else ("#f39c12" if s >= 4 else "#2ecc71")
    )
    ax.barh(rdf["name"], rdf["difficulty_score"], color=colors)
